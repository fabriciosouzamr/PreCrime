...
namespace PreCrime
{
    public partial class pagPesquisa_Listagem : System.Web.UI.Page
    {
        ...
        private void CarregarGrid(int iPessoa = 0)
        {
            ...
            sSqlText = "select cd_cpfcnpj," +
                              ...
                              "no_reduzido," +
                              "dt_resposta," +
                              "no_entrevistador_pessoa," +
                              "id_resposta ID" +
                       " from " + oDB.DBTabela("vw_pessoa_pesquisas");

            if (iPessoa != 0)
            {
                clsFuncao.Str_Adicionar(ref sSqlText_Where, "id_entrevistado = " + iPessoa.ToString(), " AND ");
            }
            if (txtCPF.Text.Trim() != "")
            {
